...
// PARTITION
Page BufferManager::getPage(string tableName, int pageIndex, int sequence)
{
    BLOCK_ACCESSES++;
    logger.log("BufferManager::getPage");
    string pageName = "../data/temp/"+ tableName  +"_Partition" + "_Page" + to_string(pageIndex)+"_S" + to_string(sequence);
    if (this->inPool(pageName))
        return this->getFromPool(pageName);
    else
        return this->insertIntoPool(tableName, pageIndex, sequence);
}
 ...
 * @return Page 
 */
Page BufferManager::insertIntoPool(string tableName, int pageIndex, int sequence)
{
    logger.log("BufferManager::insertIntoPool");
    Page page(tableName, pageIndex, sequence);
...
// matrix
void BufferManager::writePage(string tableName, int pageIndex, vector<vector<int>> rows)
{
    logger.log("BufferManager::writePageMatrix");
    Page page(tableName, pageIndex, rows);
    page.writePageMatrix();
}
// sparse matrix
void BufferManager::writePage(string tableName, int pageIndex, bool sparse, vector<vector<int>> rows)
{
    logger.log("BufferManager::writePageSparseMatrix");
    Page page(tableName, pageIndex, sparse, rows);
    page.writePageMatrix();
}
// partition
void BufferManager::writePage(string tableName, int pageIndex, int sequenceNumber, vector<vector<int>> rows, int rowCount)
{
    logger.log("BufferManager::writePage");
    Page page(tableName, pageIndex, sequenceNumber, rows, rowCount);
    page.writePage();
}
/**
 ...
 * @return Page 
 */
Page BufferManager::insertIntoMatrixPool(string matrixName, int pageIndex)
{
