...
class BufferManager{
    ...
    Page insertIntoPool(string tableName, int pageIndex, int sequence=-1);

    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex);
    Page getPage(string tableName, int pageIndex, bool isSparse);
    Page getPage(string tableName, int pageIndex, int sequence);
    ...
    void writePage(string tableName, int pageIndex, int SequenceNumber, vector<vector<int>> rows, int rowCount);
    void writePage(string tableName, int pageIndex, vector<vector<int>> rows);
    void writePage(string tableName, int pageIndex, bool sparse, vector<vector<int>> rows);
    
    Page insertIntoMatrixPool(string MatrixName, int pageIndex);
